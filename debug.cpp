...
{ 
    using namespace std;
    bool I_want_colored_output = 1; /* ONLY WORKS WITH TERMINAL */
    string white = I_want_colored_output ? "\033[0;m" : "";
    string outer = I_want_colored_output ? "\033[0;31m" : "";    // red
    string varName = I_want_colored_output ? CYAN : "";  // blue "\033[1m\033[36m"
    string varValue = I_want_colored_output ? "\033[1;32m" : ""; // green
    string anotherColor = I_want_colored_output ? MAGENTA : "";
    ...
    void printer(const char *names, T &&head, V &&...tail)
    {
        ...
        print(head);
        if (sizeof...(tail))
            cerr << outer << anotherColor <<" || ", printer(names + i + 1, tail...);
        else
            cerr << outer << "\n"
                 << white;
